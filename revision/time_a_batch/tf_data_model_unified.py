...
def load_cifar(resolution: int, dataset: str):
    ...
    if dataset not in cifar_dataset_list:
        raise ValueError(f'Invalid dataset "{dataset}", it should be in {cifar_dataset_list}.')

    resize = keras.layers.Resizing(resolution, resolution)
    ...
    # Build an unbatched base dataset. CIFAR is small enough to cache in memory.
    dataloader = {
        'train': (
...
def load_imagenet(resolution: int, dir_path: str):
    ...
    # Build an unbatched base dataset. Do not cache ImageNet because it is too large.
    dataloader = {
...
def load_data(
    ...
    """Load an unbatched base dataset for either CIFAR or ImageNet."""
    if dataset in cifar_dataset_list:
        return load_cifar(
...
def batch_dataloader(
    ...
    """Apply batching/prefetching to an already-loaded base dataset."""
