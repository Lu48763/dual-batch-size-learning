...
def get_sync_frequency_multiplier(args):
    sync_multiplier = getattr(args, 'sync_multiplier', DEFAULT_SYNC_FREQUENCY_MULTIPLIER)
    if sync_multiplier < 1:
        raise ValueError('"sync_multiplier" must be greater than or equal to 1')
    return sync_multiplier
...
class Server(object):
    def __init__(self, args):
        ...
        self.epochs = 105
        self.steps = 3
        self.sync_frequency_multiplier = get_sync_frequency_multiplier(args)
        self.validation_interval = self.sync_frequency_multiplier
        self.validation_mini_epochs = self.epochs * (args.world_size - 1)
        self.mini_epochs = self.validation_mini_epochs * self.sync_frequency_multiplier
        self.global_commit_ID = 0
        self.epoch_data_amount = 1281167
        self.total_data_amount = self.epoch_data_amount
        ...
        # Milestones
        self.validation_iter_milestones = np.array([60, 90, 105]) * (args.world_size - 1)
        self.validation_cycle_milestones = np.array([20, 40, 60, 70, 80, 90, 95, 100, 105]) * (args.world_size - 1)
        self.iter_milestones = self.validation_iter_milestones * self.sync_frequency_multiplier
        self.cycle_milestones = self.validation_cycle_milestones * self.sync_frequency_multiplier
        ...
        self.outfile = (
            ...
            f'_f{self.sync_frequency_multiplier}'
    ...
    def _update_parameter_dict(self):
        return {
            ...
            'sync_frequency_multiplier': self.sync_frequency_multiplier,
    ...
    def update_history(ps_rref, record):
        ...
        with self.history_lock:
            ...
            if record['global_commit_ID'] < self.validation_mini_epochs:
        ...
        if self.args.temp and (record['global_commit_ID'] + 1) in self.validation_iter_milestones:
...
class Worker(object):
    ...
    def train(self):
        ...
        while not self.mission_complete:
            ...
            # Train one fraction of the original mini-epoch before each sync.
            data_amount = self.parameter['small_data_amount'] if self.is_small_batch else self.parameter['large_data_amount']
            batch_size = self.parameter['small_batch_size'] if self.is_small_batch else self.parameter['large_batch_size']
            steps_per_epoch = steps_for_sync(
                data_amount=data_amount,
                batch_size=batch_size,
                sync_frequency_multiplier=self.parameter['sync_frequency_multiplier'],
            ...
            self.pending_train_loss += train_logs.history['loss'][0]
            ...
            validation_interval = self.parameter['sync_frequency_multiplier']
            if should_validate_commit(global_commit_ID, validation_interval):
                ...
                record = {
                    'worker_ID': self.rank,
                    'global_commit_ID': validation_commit_ID(global_commit_ID, validation_interval),
                    'local_commit_ID': validation_commit_ID(self.local_commit_ID, validation_interval),
