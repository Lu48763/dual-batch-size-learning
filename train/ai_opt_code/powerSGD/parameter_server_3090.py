...
class PowerSGDConfig:
    ...
    def from_args(cls, args):
        return cls(
            enabled=getattr(args, 'powersgd', True),
            rank=getattr(args, 'powersgd_rank', 1),
            start_iter=getattr(args, 'powersgd_start_iter', 2),
            min_compression_size=getattr(args, 'powersgd_min_compression_size', 4096),
            use_error_feedback=getattr(args, 'powersgd_error_feedback', True),
            seed=getattr(args, 'powersgd_seed', 48763),
        )


def _matrix_view(array):
    if array.ndim < 2:
        return None
    rows = int(np.prod(array.shape[:-1]))
    cols = array.shape[-1]
    return array.reshape(rows, cols)


def _raw_payload(array):
    return {
        'kind': 'raw',
...
def _low_rank_payload(array, config, iteration, tensor_index):
    array = np.asarray(array)
    matrix = _matrix_view(array.astype(np.float32, copy=False))
    if matrix is None:
        return _raw_payload(array)

    rank = min(config.rank, matrix.shape[0], matrix.shape[1])
    compressed_size = (matrix.shape[0] + matrix.shape[1]) * rank
    if rank < 1 or array.size < config.min_compression_size or compressed_size >= array.size:
        return _raw_payload(array)

    rng = np.random.default_rng(config.seed + iteration * 1000003 + tensor_index)
    q = rng.standard_normal((matrix.shape[1], rank)).astype(np.float32)
    q, _ = np.linalg.qr(q)
    p = matrix @ q
    p, _ = np.linalg.qr(p)
    q = matrix.T @ p
...
def decompress_powerSGD_payload(payloads):
    arrays = []
    for payload in payloads:
        if payload['kind'] == 'raw':
            arrays.append(np.asarray(payload['array']).copy())
        elif payload['kind'] == 'low_rank':
            matrix = payload['p'] @ payload['q'].T
            array = matrix.reshape(payload['shape']).astype(payload['dtype'], copy=False)
            arrays.append(array)
        else:
            raise ValueError(f'Invalid PowerSGD payload kind "{payload["kind"]}".')
    return arrays
...
def build_worker_payload(worker_weights, global_weights, config, iteration, error_feedback=None):
        ...
        with self.parameter_lock:
            with self.global_model_lock:
                if not self.mission_complete:
                    ...
                    if is_powerSGD_payload(worker_weights) or len(worker_weights) > 1:
                        ...
                        if is_powerSGD_payload(worker_weights):
                            server_weights = apply_powerSGD_payload(server_weights, worker_weights, update_factor)
                        else:
    ...
    def train(self):
        ...
        while not self.mission_complete:
            ...
            worker_payload, self.powerSGD_error_feedback = build_worker_payload(
                ...
                global_weights,
                config=self.powerSGD_config,
                iteration=self.local_commit_ID,
                error_feedback=self.powerSGD_error_feedback,
            )
