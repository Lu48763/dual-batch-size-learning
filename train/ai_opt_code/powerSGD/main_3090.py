...
parser.add_argument(
    ...
    dest='powersgd',
    action='store_false',
    help='disable PowerSGD-style compression and send full worker weights',
)
parser.add_argument(
    '--powersgd-rank',
    type=int,
    default=1,
    help='low-rank approximation rank for PowerSGD-style compressed updates',
)
parser.add_argument(
    '--powersgd-start-iter',
    type=int,
...
parser.add_argument(
    ...
    type=int,
    default=4096,
    help='minimum tensor element count required before applying PowerSGD compression',
)
parser.add_argument(
    '--no-powersgd-error-feedback',
    dest='powersgd_error_feedback',
    action='store_false',
    help='disable PowerSGD residual error feedback on workers',
)
parser.add_argument(
    '--powersgd-seed',
    type=int,
...
def main():
    ...
    if args.powersgd_start_iter < 0:
        raise ValueError('"powersgd_start_iter" must be greater than or equal to 0')
    if args.powersgd_min_compression_size < 1:
        raise ValueError('"powersgd_min_compression_size" must be greater than or equal to 1')
    print('----')
    print(args)
    print('----')
